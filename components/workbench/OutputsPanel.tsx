...
export function OutputsPanel({ visible, sessionId, title, leafId, refreshKey, onOpen, onMessage }: { visible: boolean; sessionId: string | null; title?: string; leafId: string | null; refreshKey: number; onOpen: (path: string) => void; onMessage: (entryId: string, leafId: string | null) => void }) {
  ...
  return <section className="workbench-content" aria-label={t("wb.outputs")}><h2 className="workbench-sr-only">{t("wb.outputs")}</h2><small>{title || t("wb.selectSession")} · {t("wb.selectedBranch")}</small><p>{t("wb.currentFileHint")}</p>{error && <p role="alert">{error}</p>}
    {sessionId && <><div className="workbench-toolbar"><input aria-label={t("wb.searchOutputs")} placeholder={t("wb.searchOutputs")} value={search} onChange={e => setSearch(e.target.value)} /><button onClick={() => refresh(v => v + 1)}>{t("wb.refresh")}</button></div><select aria-label={t("wb.filter")} value={filter} onChange={e => setFilter(e.target.value)}>{["all", "documents", "images", "audio", "other"].map(v => <option key={v} value={v}>{t(`wb.${v}`)}</option>)}</select><label><input type="checkbox" checked={hidden} onChange={e => setHidden(e.target.checked)} />{t("wb.showHidden")}</label>
    {!current ? <p>{t("wb.loading")}</p> : !current.items.length ? <p>{t("wb.noOutputs")}</p> : current.items.filter(i => (hidden || !i.hidden) && (filter === "all" || filter === i.kind) && `${i.label} ${i.path}`.toLowerCase().includes(search.toLowerCase())).map(item => <article className="workbench-card" key={item.id}>
