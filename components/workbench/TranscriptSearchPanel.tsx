...
export function TranscriptSearchPanel({ visible, onOpen }: { visible: boolean; onOpen: (result: TranscriptResult, query: string, signal: AbortSignal) => Promise<void> }) {
  ...
  return <section className="workbench-content transcript-search" aria-label={t("wb.transcriptSearch")}>
    <form onSubmit={event => { event.preventDefault(); void search(); }}>
      <div className="workbench-toolbar"><input aria-label={t("wb.transcriptSearch")} placeholder={t("wb.searchPlaceholder")} value={query} maxLength={256} onChange={event => { invalidate(); setQuery(event.target.value); }} onKeyDown={event => { if (event.key !== "Enter" || event.nativeEvent.isComposing) return; event.preventDefault(); if (query.trim() && !busy) void search(); }} /><button disabled={!query.trim() || busy}>{t("wb.search")}</button>{busy && <button type="button" onClick={() => { request.current?.abort(); generation.current++; setBusy(false); }}>{t("wb.cancel")}</button>}</div>
