...
test("renders notices in mutually exclusive empty and transcript layouts", async () => {
  assert.match(source, /\{isEmptyNew \? \(/);
  assert.equal((source.match(/<NoticeShelf notices=\{notices\}/g) ?? []).length, 2);
  assert.match(source, /<NoticeShelf notices=\{notices\} floating onPauseChange=\{setNoticePaused\}/);
});
