...
test("floats the scroll-to-latest button above the composer", () => {
  // The fork anchors the button with a class (no minimap column to clear);
  ...
  const anchor = cssSource.slice(cssSource.indexOf(".chat-scroll-to-bottom-anchor {"));
  assert.match(anchor.slice(0, anchor.indexOf("}")), /position: absolute;[\s\S]*?bottom: 100%;[\s\S]*?justify-content: center;[\s\S]*?pointer-events: none;/);
...
test("exposes the detached-tail flag from the session hook without a ref read", () => {
  ...
  assert.match(hookSource, /isNew,\s*showScrollToBottom,\s*\/\/ Refs/);
});
