...
test("shows the follow-up shortcut in the button tooltip", () => {
  ...
  assert.match(html, /aria-keyshortcuts="Enter Alt\+Enter"/);
});
...
test("file mention menu applies the measured upward height cap", () => {
  ...
  assert.match(block, /composer-completion-popover/);
  assert.match(nativeThemeSource, /\.composer-completion-popover[\s\S]*?flex-direction: column/);
...
test("locks built-in command submission until it settles", async () => {
  ...
  function findCallback(node) {
    if (ts.isVariableDeclaration(node) && node.name.getText(source) === "dispatchBuiltin") {
      return node.initializer.arguments[0];
    }
  ...
  const callback = new Script(ts.transpileModule(findCallback(source).getText(source), {
    ...
    parseWebSlashCommand: (text) => text === "/reload" ? { name: "reload", idle: true } : null,
    snapshotRef: { current: () => ({ value: "/reload", images: [] }) },
    draftKeyRef: { current: "draft" }, isStreaming: false, setPreparationError() {},
    t: key => key,
    attachedImages: [],
  ...
  assert.match(sourceText, /disabled=\{builtinCommandPending/);
});
