...
test("fresh sessions use the preference while persisted and live sessions restore their selection", async () => {
  assert.match(source, /isNew \? loadStoredToolPreset\(\) : "default"/);
  assert.match(source, /setToolPresetState\(d.toolNames !== undefined \? getPresetFromToolNames\(d.toolNames\)/);
  assert.match(source, /getPresetFromToolNames/);
});
