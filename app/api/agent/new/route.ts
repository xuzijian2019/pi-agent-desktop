...
export async function POST(req: Request) {
  try {
    ...
    const { provider, modelId, toolNames, thinkingLevel, persistPreferences, ...promptCommand } = command as { provider?: string; modelId?: string; toolNames?: string[]; thinkingLevel?: unknown; [key: string]: unknown };
    ...
    const { session, realSessionId } = await startRpcSession(tempKey, "", cwd, {
      ...(toolNames ? { toolNames } : {}),
      ...(persistPreferences === false ? { persistPreferences: false } : {}),
