...
test("the published package ships the handoff helper", () => {
  const pkg = JSON.parse(readFileSync(new URL("../package.json", import.meta.url), "utf8"));
  assert.ok(pkg.files.includes("lib/browser-open.js"));
});

test("pi-web.js uses the shared helper instead of xdg-open or cmd start", () => {
  const source = readFileSync(new URL("../bin/pi-web.js", import.meta.url), "utf8");
  assert.match(source, /attachReadyHandoff/);
  assert.match(source, /wireChildProcessLifecycle/);
  assert.equal(source.includes("xdg-open"), false);
  assert.doesNotMatch(source, /shell:\s*isWindows/);
});

test("npm run dev still does not auto-open", () => {
  const pkg = JSON.parse(readFileSync(new URL("../package.json", import.meta.url), "utf8"));
