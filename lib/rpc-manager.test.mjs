...
test("RPC sessions explicitly direct file mutations to Pi's edit tool", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));

  assert.match(source, /EDIT_TOOL_PRIORITY_INSTRUCTION/);
  assert.match(source, /Do not run apply_patch through bash/);
  assert.match(startupSource, /appendSystemPromptOverride: \(base\) => \[\.\.\.base, EDIT_TOOL_PRIORITY_INSTRUCTION\]/);
});
