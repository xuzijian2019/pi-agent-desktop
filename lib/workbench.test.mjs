...
test("the right panel offers four modes and degrades retired ones to Files", async () => {
  const { PANEL_MODES, panelMode } = await jiti.import("./panel-modes.ts");
  assert.deepEqual([...PANEL_MODES], ["files", "activity", "search", "tasks"]);
  // Workspaces persisted before the 8 -> 4 consolidation must still open.
  for (const retired of ["browser", "diff", "outputs", "context", "", null, undefined, 3]) {
    assert.equal(panelMode(retired), "files");
  }
  for (const kept of PANEL_MODES) assert.equal(panelMode(kept), kept);
});

test("the send preview popover stays above the composer and inside its width", async () => {
