...
test("counts standalone usage entries (cache warming) in file totals", () => {
  const cacheWarm = {
    type: "usage",
    kind: "cache_warm",
    provider: "anthropic",
    ...
    parentId: null,
    timestamp: "2026-01-01T00:00:00.000Z",
    usage: usage({ input: 500, cacheRead: 900, cost: { input: 0, output: 0, cacheRead: 0.9, cacheWrite: 0, total: 0.9 } }),
  };
  const without = computeSessionStats([userEntry("u1", null), assistantEntry("a1", "u1", [{ type: "text", text: "hi" }], usage())]);
  const withWarm = computeSessionStats([userEntry("u1", null), cacheWarm, assistantEntry("a1", "u1", [{ type: "text", text: "hi" }], usage())]);

  assert.equal(withWarm.tokens.input - without.tokens.input, 500);
  assert.equal(withWarm.tokens.cacheRead - without.tokens.cacheRead, 900);
  assert.ok(Math.abs(withWarm.cost - without.cost - 0.9) < 1e-9);
  // Usage records are not messages.
  assert.equal(withWarm.totalMessages, without.totalMessages);
});
