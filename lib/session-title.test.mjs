...
test("names a running session from its pending prompt when waitForIdle is false", async () => {
  let waited = false;
  ...
  const sourceAgent = {
    ...
    streamFunction: (_model, context) => {
      providerMessages = context.messages.slice();
      const stream = createAssistantMessageEventStream();
      queueMicrotask(() => {
        stream.push({ type: "done", reason: "stop", message: assistantMessage("Implement automatic session naming") });
      });
      return stream;
    },
  };

  const result = await generateSessionTitle({ agent: sourceAgent }, { waitForIdle: false });

  assert.equal(waited, false);
  assert.equal(result.title, "Implement automatic session naming");
  // pi 0.86 materializes the system prompt as the leading transcript message.
  assert.deepEqual(providerMessages.map((message) => message.role), ["system", "user"]);
  assert.match(providerMessages[1].content, /Implement auto name[\s\S]*Do not answer it[\s\S]*Create a concise title/);
});
