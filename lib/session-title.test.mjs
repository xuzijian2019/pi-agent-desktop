...
test("names a running session from what it holds right now, without waiting for idle", async () => {
  ...
  // pi >= 0.86 stream functions take a TranscriptContext: the title system prompt
  // rides as the leading system message, with no tool declarations attached.
  ...
  assert.equal(seen.messages[0].toolsAdded, undefined);
