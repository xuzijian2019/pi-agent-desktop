...
const nextTurn = () => new Promise((resolve) => realSetTimeout(resolve, 50));
...
function makePromptInner(prompt) {
  return {
    ...
    isStreaming: false,
    extensionRunner: {},
    sessionManager: { getCwd: () => "/tmp", getSessionName: () => "Test session", getHeader: () => null, getEntries: () => [] },
    agent: { state: {} },
...
test("an exact system prompt is reapplied to the Pi 0.86 transcript after preflight", async (t) => {
  const { Agent } = await jiti.import("@earendil-works/pi-agent-core");
  const { getCurrentSystemPrompt, normalizeContext } = await jiti.import("@earendil-works/pi-ai");
  const inner = makePromptInner((_message, options) => {
    inner.agent.state.messages = normalizeContext({ systemPrompt: "SDK base prompt", messages: [] }).messages;
    options.preflightResult(true);
    assert.equal(inner.agent.state.systemPrompt, "context prompt");
    return Promise.resolve();
  });
  inner.agent = new Agent({ initialState: { systemPrompt: "initial SDK prompt" } });
  const wrapper = new AgentSessionWrapper(inner, { exactSystemPrompt: () => "context prompt", chatOnly: true });
  t.after(() => wrapper.destroy());
  assert.equal(inner.agent.state.systemPrompt, "context prompt");
  await wrapper.send({ type: "prompt", message: "hello" });
  await nextTurn();
  const prepared = await inner.agent.prepareNextTurnWithContext({
    context: normalizeContext({ systemPrompt: "SDK continuation prompt", messages: [] }),
  });
  assert.equal(getCurrentSystemPrompt(prepared.context.messages), "context prompt");
});
